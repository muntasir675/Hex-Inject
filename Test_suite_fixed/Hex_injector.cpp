...
void Warnings(ifstream &file)
{
    ...
    while (getline(file, line))
    {
        ...
        if (line.length() < 11)
            cout << "         [Line " << line_num << "] Line too short\n";
        else
        {
            if (line.length() != 11 + byte_count * 2)
                cout << "          [Line " << line_num << "] Byte count mismatch\n";
            if (!verify_checksum(line))
                cout << "          [Line " << line_num << "] Checksum error\n";
            if (line.substr(7, 2) == "01")
                eof_found = true;
        }
    }
    if (!eof_found)
        cout << "          [Line " << line_num << "] End of file record not found\n";
}

void process(ifstream &file, string &serial_num, const string &file_name)
{
    string line, address, record, upper_address = "";
    int byte_count, real_address;
    bool ph = 0, overwrite = (bool)ifstream("Injected_" + file_name + ".hex");
    ...
    if (ph)
    {
        ...
        cout << "Status:   Injected successfully\n";
        cout << "Output:   " << (overwrite ? "Overwritten: " : "Created: ") << "Injected_" << file_name << ".hex\n\n";
    }
    else
    {
        cout << "Status:   Serial number not found at address 0x2000\n\n";
    }
}
...
int main(int argc, char *argv[])
{
    ...
    else if (argc == 3)
    {
        file_name = argv[1];
        serial_num = argv[2];
        if(!check_file(file, file_name) || !check_serial(serial_num))
            exit(1);
        cout << "File:     " << file_name << ".hex\n";
        cout << "Serial:   " << serial_num << "\n";
        cout << "Warnings:\n";
        Warnings(file);
        process(file, serial_num, file_name);
    }
    else if (argc == 2)
    {
        file_name = argv[1];
